...
ip addr add $LOOPBACK_ADDR dev br0
ip link set br0 up

ovs-vsctl del-port --if-exists gateway

if [ ! -z "$IS_GATEWAY" ]
then
	ovs-vsctl add-port br0 gateway \
		-- set interface gateway type=internal
	ip addr add $IS_GATEWAY dev gateway
